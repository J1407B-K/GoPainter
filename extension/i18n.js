...
(() => {
  ...
  const en = {
    ...
    '开始体检全部规则集': 'Check all rule sets', '规则详情': 'Rule details', '关闭': 'Close', '保存并生效': 'Save & apply',
    ...
    '清空历史': 'Clear history', '目标 URL': 'Target URLs', '起始 URL': 'Start URL', '最大页数': 'Maximum pages',
    '开始批量扫描': 'Start batch scan', '清空结果': 'Clear results', '开始爬取': 'Start crawl', '停止': 'Stop', '模型': 'Model',
    ...
    '实时 YAML 校验 · Go/WASM': 'Live YAML validation · Go/WASM', '检查 YAML': 'Check YAML', '正在校验 YAML…': 'Validating YAML…',
    'YAML / 规则无效：': 'Invalid YAML / rule: ', 'YAML / 规则无效': 'Invalid YAML / rule',
    '✓ YAML 与规则有效 · 当前页面命中': '✓ Valid YAML and rule · matches the current page',
    '✓ YAML 与规则有效 · 当前页面未命中': '✓ Valid YAML and rule · does not match the current page',
    '✓ YAML 与规则有效 · JS/DOM 探针将在保存后重新采集': '✓ Valid YAML and rule · JS/DOM probes will be recollected after saving',
    ...
    '以下固定来源由各自社区维护。GoPainter 会有界下载、转换并原子替换各自的独立规则集；': 'These fixed sources are maintained by their communities. GoPainter downloads them within strict bounds, converts them, and atomically replaces each source’s independent rule set.',
    '可手动刷新或定时检查（默认关闭），并保留一个可回滚版本。': 'Refresh manually or enable scheduled checks (off by default); one rollback version is retained.',
    '以下固定来源由各自社区维护。GoPainter 会有界下载、转换并原子替换各自的独立规则集； 可手动刷新或定时检查（默认关闭），并保留一个可回滚版本。': 'These fixed sources are maintained by their communities. GoPainter downloads them within strict bounds, converts them, and atomically replaces each source’s independent rule set. Refresh manually or enable scheduled checks (off by default); one rollback version is retained.',
    '规则数据不随本扩展分发': 'Rule data is not distributed with this extension',
    '。 第三方内容、版权与合规性由其维护者和使用者自行负责。': '. Third-party content, licensing, and compliance are the responsibility of its maintainers and users.',
    ...
    '立即刷新': 'Refresh now', '更新中…': 'Updating…', '回滚': 'Roll back', '自动更新': 'Auto-update',
    '每天': 'Daily', '每周': 'Weekly', '尚未同步': 'Never synced',
    '958 条指纹，国产系统覆盖好（关键词 + faviconhash）': '958 fingerprints, with strong coverage of Chinese products (keywords + favicon hashes)',
    ...
    '保留页面/批量/爬取扫描的摘要：URL、标题、状态码、favicon 哈希和命中证据。不保存页面 HTML、响应头或 API Key。': 'Stores page, batch, and crawl summaries: URL, title, status, favicon hashes, and match evidence. It does not store page HTML, response headers, or API keys.',
    '每行一个 http/https URL，自动去重；最多 500 个，并发上限 4。页面响应体只读取前 200 KB，结果以紧凑摘要保存在当前浏览器会话。批量扫描不会打开标签页或执行页面 JS/DOM。': 'Enter one http/https URL per line. URLs are deduplicated; the limit is 500 targets with concurrency capped at 4. Only the first 200 KB of each response is read, and compact results are kept for the current browser session. Batch scans do not open tabs or execute page JavaScript/DOM probes.',
    ...
    '尚无扫描记录': 'No scan history yet', '尚未扫描': 'No batch scan yet', '未知时间': 'Unknown time', '页面': 'Page', '批量': 'Batch', '爬取': 'Crawl',
    ...
    '保存规则失败': 'Failed to save rule', '读取规则失败': 'Failed to load rule', '规则校验失败': 'Rule validation failed',
    '规则规范化失败': 'Rule normalization failed',
    '规则入库失败': 'Failed to add rule', '规则转换失败': 'Rule conversion failed',
    ...
    '生成中…': 'Generating…', '优化中…': 'Optimizing…', '新建规则': 'New rule', '编辑规则': 'Edit rule', '优化此规则': 'Optimize this rule',
    '点击编辑此规则': 'Click to edit this rule', '正在校验…': 'Validating…',
    '规则有效': 'Valid rule', '规则无效': 'Invalid rule',
    ...
    '拉取中…': 'Fetching…', '转换中…': 'Converting…', '正在检查并转换…': 'Checking and converting…',
    '正在下载、转换并验证规则…': 'Downloading, converting, and validating rules…',
    '远程规则没有变化': 'Remote rules are unchanged', '规则源更新完成': 'Rule source updated',
    '规则源已回滚': 'Rule source rolled back', '已开启自动更新': 'Auto-update enabled',
    '已关闭自动更新': 'Auto-update disabled', '从未': 'Never',
    '确定将此规则源回滚到上一个版本？当前版本会成为新的回滚版本。': 'Roll this source back to its previous version? The current version will become the new rollback version.',
    '加载规则源失败': 'Failed to load rule sources', '规则源更新失败': 'Rule source update failed',
    '规则源回滚失败': 'Rule source rollback failed', '保存自动更新设置失败': 'Failed to save auto-update settings',
    '规则源重定向到了不允许的地址': 'The rule source redirected to a disallowed address',
    '规则源重定向次数过多': 'The rule source exceeded the redirect limit',
    '规则源重定向缺少 Location': 'The rule-source redirect is missing a Location header',
    '规则源响应超过大小上限': 'The rule-source response exceeds the size limit',
    '规则源响应没有正文': 'The rule-source response has no body',
    '规则源下载失败': 'Rule-source download failed', 'Wappalyzer 文件列表异常': 'The Wappalyzer file list is invalid',
    'nuclei 文件列表异常': 'The nuclei file list is invalid', 'EHole fingerprint 不是数组': 'The EHole fingerprint payload is not an array',
    '规则源备份事务已中止': 'The rule-source backup transaction was aborted',
    '未知规则源': 'Unknown rule source', '没有可回滚的规则版本': 'No rule-source version is available to roll back',
    '远程规则集无效': 'The downloaded rule set is invalid',
    ...
    '已有同名规则集': 'A rule set with this name already exists', '已有爬取任务在跑': 'A crawl is already running', '已有批量扫描任务在跑': 'A batch scan is already running',
    '没有有效的 http/https URL': 'No valid http/https URLs', '请先停止批量扫描': 'Stop the batch scan first',
    '启动批量扫描失败': 'Failed to start batch scan', '清空批量扫描结果失败': 'Failed to clear batch results',
    '编辑器必须包含且只能包含一条规则': 'The editor must contain exactly one rule',
    '当前编辑集不存在': 'The active rule set does not exist',
    '规则状态已更新，请重新打开编辑器后重试': 'Rule state changed; reopen the editor and try again',
  ...
  const patterns = [
    [/^批量扫描中：(\d+) \/ (\d+)，等待 (\d+)，失败 (\d+)$/, (_, done, total, pending, failed) => `Batch scanning: ${done} / ${total}, ${pending} pending, ${failed} failed`],
    [/^批量扫描被系统中断：已完成 (\d+) \/ (\d+)$/, (_, done, total) => `Batch scan interrupted: ${done} / ${total} completed`],
    [/^批量扫描异常：(.*)$/, (_, error) => `Batch scan error: ${error}`],
    [/^批量扫描完成(（已停止）)?：成功 (\d+)，失败 (\d+)，无效 (\d+)，重复 (\d+)$/, (_, stopped, ok, failed, invalid, duplicate) => `Batch scan ${stopped ? 'stopped' : 'finished'}: ${ok} succeeded, ${failed} failed, ${invalid} invalid, ${duplicate} duplicates`],
    [/^已开始扫描 (\d+) 个 URL$/, (_, count) => `Started scanning ${count} URLs`],
    [/^批量扫描最多 (\d+) 个 URL$/, (_, count) => `Batch scans support at most ${count} URLs`],
    [/^规则 (.+) 已保存并生效$/, (_, id) => `Rule ${id} saved and applied`],
    ...
    [/^上次检查失败：(.*)$/, (_, error) => `Last check failed: ${translate(error)}`],
    [/^(\d+) 条规则；上次检查：(.*?)(?:；上次变化：新增 (\d+)，删除 (\d+)，修改 (\d+))?$/, (_, count, checked, added, removed, changed) => `${count} rules; last checked: ${checked}${added === undefined ? '' : `; last change: ${added} added, ${removed} removed, ${changed} changed`}`],
    [/^规则源更新失败：(.*)$/, (_, error) => `Rule source update failed: ${translate(error)}`],
    [/^规则源回滚失败：(.*)$/, (_, error) => `Rule source rollback failed: ${translate(error)}`],
    [/^保存自动更新设置失败：(.*)$/, (_, error) => `Failed to save auto-update settings: ${translate(error)}`],
    [/^转换后的规则数量异常：(\d+)$/, (_, count) => `Unexpected converted rule count: ${count}`],
    [/^(.+) JSON 无效：(.*)$/, (_, label, error) => `${label} JSON is invalid: ${error}`],
    ...
    [/^规则 (.+) 无法编辑$/, (_, id) => `Rule ${id} cannot be edited`],
    ...
    [/^(页面|批量|爬取) · (.*)（HTTP (.*)）$/, (_, kind, url, status) => `${kind === '页面' ? 'Page' : kind === '批量' ? 'Batch' : 'Crawl'} · ${url} (HTTP ${status})`],
    [/^编辑规则必须保留原 id：(.+)$/, (_, id) => `The edited rule must keep its original ID: ${id}`],
  ...
  const attrs = {
    ...
    '设置分区': 'Settings sections', '点击查看该规则': 'View this rule', '规则 YAML': 'Rule YAML', '删除': 'Delete',
    'Wappalyzer 更新频率': 'Wappalyzer update frequency', 'EHole 更新频率': 'EHole update frequency',
    'nuclei 更新频率': 'nuclei update frequency',
  };
