...
(() => {
  ...
  async function convertWappalyzer(entries) {
    const rules = [];
  ...
  async function convertEHole(fingers) {
    const rules = [];
  ...
  async function normalizeDocs(docs) {
    const rules = [];
  ...
  async function fetchWappalyzer(def, meta, budget) {
    ...
    const documents = await mapConcurrent(files, async (file) => {
      const response = await fetchBounded(file.download_url, budget);
      return parsedJSON(response.text, file.name);
    });
    const technologies = Object.assign({}, ...documents);
    return { ...manifest, rules: await convertWappalyzer(Object.entries(technologies)) };
  }

  async function fetchEHole(def, meta, budget) {
    ...
    return { ...response, rules: await convertEHole(fingers) };
  }

  async function fetchNuclei(def, meta, budget) {
    const manifest = await fetchBounded(def.manifest, budget, {
      maxBytes: 2 * 1024 * 1024, etag: meta.etag, lastModified: meta.lastModified,
    });
    if (manifest.notModified) return manifest;
    ...
    return { ...manifest, rules: await normalizeDocs(documentLists.flat()) };
  }
  ...
  async function performUpdate(sourceId) {
    ...
    let fetched;
    if (sourceId === 'wappalyzer') fetched = await fetchWappalyzer(def, meta, budget);
    else if (sourceId === 'ehole') fetched = await fetchEHole(def, meta, budget);
    else fetched = await fetchNuclei(def, meta, budget);
  ...
  function updateSource(sourceId) {
    ...
    const task = performUpdate(sourceId).catch(async (error) => {
