...
本版扩展了规则集管理：可同时启用多个规则集参与匹配，编辑操作仍限定在当前编辑集；导入或 AI 优化遇到同 ID 冲突时，会先展示 YAML diff，由用户明确选择是否覆盖。同时修复 nuclei 模板导入，把大型规则集启用工作移出设置页主线程，并支持导出当前编辑集的 YAML。最近一次已发布的性能测量仍可见[性能记录](BENCHMARK_CN.md)。
