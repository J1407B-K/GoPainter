...
v0.7.1 增加首次安装即可本地体验的规则示例，更清晰的 YAML 编辑与校验，以及更顺滑的
popup 命中展示。第三方规则源在手动和定时更新间保持稳定规则集标识；其完整已安装规则也
可重新打开并保存，不再误受 AI 候选规则 50 个 matcher 的限制。设置页可批量扫描最多 500
个 URL，并导出 JSON/CSV；批量扫描固定为 4 路并发，页面响应按字节流截断，session 结果
使用独立的 2.5 MB 预算。
...
可靠采集、匹配、校验和结果工作流；v0.7.1 不扩充指纹规则内容。实际使用的规则由使用者
导入、编辑，或主动从兼容来源更新。
