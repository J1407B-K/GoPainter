...
v0.6.5 加固了页面探测、favicon 哈希、规则状态一致性与缓存失效：运行时探测改走扩展侧 MAIN world bridge；icon 下载具备严格的 URL 与字节预算；规则修改串行化并加入 revision 校验。扩展界面同时加入了持久化的中英切换。当前测量及各历史版本见[性能记录](BENCHMARK_CN.md)。
...
浏览器核心工作流已经完整：自动匹配与证据、规则集组合启用和导入、Agent 研究与可导入规则生成、站点爬取、历史/报告导出和书签整理。下一阶段计划是远程规则源订阅与更新。
