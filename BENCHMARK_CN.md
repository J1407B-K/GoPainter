...
v0.5.0 将既有的 body AC 索引扩展到 regex 的必需字面量，并加入保守的 AST 分支分析、Unicode 安全边界和 evidence / 结果路径的分配优化。连续扫描数据还证明：即使匹配工作显著减少，TinyGo 的 GC p99 仍比标准 Go 高 7–10 倍；因此生产构建切换为标准 Go WASM。
...
| TinyGo 为默认构建 | 标准 Go 因尾延迟稳定成为生产默认 |
...
即使预筛移除了绝大多数正则工作，TinyGo 仍会出现 GC 尾延迟尖峰。因此标准 Go 是生产默认；若二进制体积比尾延迟更重要，仍可用 `make build-tinygo`。
...
上述真实规则数据来自 Chrome、用户导入的规则集和真实中文页面。规则库有实质变化后，应重新测量并将新的对比追加在本文件中，而不是覆盖 v0.5.0 的记录。
