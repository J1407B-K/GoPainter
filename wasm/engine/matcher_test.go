...
func baseFeatures() *Features {
	return &Features{
		...
		Dom:         []string{".wp-block", "#login-form"},
	}
}

func TestEvalMatcherWord(t *testing.T) {
	f := baseFeatures()
	ok, ev := evalMatcher(Matcher{Type: "word", Words: []string{"POWERED BY"}}, f)
	if !ok {
...
func TestEvalMatcherWordPart(t *testing.T) {
	f := baseFeatures()
	ok, _ := evalMatcher(Matcher{Type: "word", Part: "title", Words: []string{"login"}}, f)
	if !ok {
		t.Error("title part 应命中")
	}
	ok, _ = evalMatcher(Matcher{Type: "word", Part: "header", Words: []string{"server: nginx"}}, f)
	if !ok {
		t.Error("header part 应命中")
	}
	ok, _ = evalMatcher(Matcher{Type: "word", Part: "meta", Words: []string{"generator: wordpress 6.5"}}, f)
	if !ok {
		t.Error("meta part 应命中")
	}
	ok, _ = evalMatcher(Matcher{Type: "word", Part: "script", Words: []string{"/wp-content/theme.js"}}, f)
	if !ok {
		t.Error("script part 应命中")
	}
	ok, _ = evalMatcher(Matcher{Type: "word", Part: "url", Words: []string{"/login"}}, f)
	if !ok {
...
func TestEvalMatcherWordNegative(t *testing.T) {
	f := baseFeatures()
	ok, _ := evalMatcher(Matcher{Type: "word", Words: []string{"nginx"}, Negative: true}, f)
	if ok {
		t.Error("negative：命中内容应不匹配")
	}
	ok, ev := evalMatcher(Matcher{Type: "word", Words: []string{"tomcat"}, Negative: true}, f)
	if !ok {
		t.Error("negative：内容缺席应匹配")
	}
	if len(ev) == 0 || !strings.Contains(ev[0].Detail, "未出现") {
		t.Errorf("negative 证据应标注未出现: %+v", ev)
	}
}

func TestEvalMatcherRegex(t *testing.T) {
	f := baseFeatures()
	ok, ev := evalMatcher(Matcher{Type: "regex", Regex: []string{`Powered\s+by`}}, f)
	if !ok {
		t.Error("regex 应命中")
	}
...
func TestEvalMatcherRegexInvalid(t *testing.T) {
	f := baseFeatures()
	// 非法正则不应 panic，应判不匹配
	ok, _ := evalMatcher(Matcher{Type: "regex", Regex: []string{`([unclosed`}}, f)
	if ok {
		t.Error("非法正则应判不匹配")
	}
}
...
func TestEvalMatcherStatus(t *testing.T) {
	f := baseFeatures()
	ok, _ := evalMatcher(Matcher{Type: "status", Status: []int{200}}, f)
	if !ok {
		t.Error("status 200 应命中")
	}
	ok, _ = evalMatcher(Matcher{Type: "status", Status: []int{301, 500}}, f)
	if ok {
...
func TestEvalMatcherIconHash(t *testing.T) {
	f := baseFeatures()
	ok, _ := evalMatcher(Matcher{Type: "icon_hash", Hash: []int32{12345}}, f)
	if !ok {
		t.Error("主 favicon 哈希应命中")
	}
	// 多 favicon：主哈希不中，但额外列表里有
	ok, _ = evalMatcher(Matcher{Type: "icon_hash", Hash: []int32{999}}, &Features{FaviconHashes: []int32{999, 12345}})
	if !ok {
		t.Error("额外 favicon 哈希应命中")
	}
	ok, _ = evalMatcher(Matcher{Type: "icon_hash", Hash: []int32{777}}, f)
	if ok {
		t.Error("哈希不匹配应判假")
	}
}
...
func TestEvalMatcherDsl(t *testing.T) {
	f := baseFeatures()
	ok, _ := evalMatcher(Matcher{Type: "dsl", Dsl: []string{`contains(body, "Nginx") && status == 200`}}, f)
	if !ok {
		t.Error("dsl 命中条件应通过")
	}
	ok, _ = evalMatcher(Matcher{Type: "dsl", Dsl: []string{`contains(body, "不存在")`}}, f)
	if ok {
		t.Error("dsl 未命中应判假")
	}
}
...
func TestEvalMatcherJs(t *testing.T) {
	f := baseFeatures()
	ok, ev := evalMatcher(Matcher{Type: "js", Js: []JsProbe{{Path: "React.version"}}}, f)
	if !ok {
	...
	// 带 pattern：值要匹配
	ok, _ = evalMatcher(Matcher{Type: "js", Js: []JsProbe{{Path: "React.version", Pattern: `^18\.`}}}, f)
	if !ok {
		t.Error("js pattern 命中应通过")
	}
	ok, _ = evalMatcher(Matcher{Type: "js", Js: []JsProbe{{Path: "React.version", Pattern: `^19\.`}}}, f)
	if ok {
		t.Error("js pattern 不匹配应判假")
	}
	ok, _ = evalMatcher(Matcher{Type: "js", Js: []JsProbe{{Path: "NotExist"}}}, f)
	if ok {
		t.Error("js 变量不存在应判假")
	}
}
...
func TestEvalMatcherDom(t *testing.T) {
	f := baseFeatures()
	ok, _ := evalMatcher(Matcher{Type: "dom", Words: []string{".wp-block"}}, f)
	if !ok {
		t.Error("dom 裸选择器应命中")
	}
	ok, _ = evalMatcher(Matcher{Type: "dom", Dom: []DomProbe{{Sel: "#login-form"}}}, f)
	if !ok {
		t.Error("dom probe 应命中")
	}
	ok, _ = evalMatcher(Matcher{Type: "dom", Dom: []DomProbe{{Sel: ".nope"}}}, f)
	if ok {
